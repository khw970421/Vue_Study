<template>
    <li>
        <div    :style="{paddingLeft : `${14 * depth}px`}"
...
<!--                @click="$router.push(`/workspaces/${workspace.id}`)">-->
                ...
                <span   :class="{active : showChildren}"
            ...
            <div class="actions">
                <span class="material-icons" @click.stop="createWorkspace">
                    add
                </span>
        ...
        <div v-if = "!hasChildren && showChildren" class="no-children"
            :style="{paddingLeft:`${14 * depth + 22}px`}">하위 페이지가 없습니다.</div>
        <ul v-if="hasChildren && showChildren">
            <WorkspaceItem v-for="ws in workspace.documents"
                            :key="ws.id"
                           :workspace="ws"
                            :depth="depth+1"></WorkspaceItem>
        </ul>
    </li>
</template>
